...
namespace TestProject.Tests
{
    public class DocumentServiceTests
    {
        ...
        [Fact]
        public async Task SearchAsync_ShouldFindAndHighlightDocuments()
        {
            var (adminService, documentService) = CreateServices();
            ...
            await documentService.AddAsync(new DocumentModel
            {
                ...
                Author = "Author1",
                Year = "2025",
                Content = "Это первый тестовый документ для поиска.",
                CloudLink = "link1"
            });
            ...
            await documentService.AddAsync(new DocumentModel
            {
                ...
                Author = "Author2",
                Year = "2025",
                Content = "Второй документ содержит несколько слов для поиска.",
                CloudLink = "link2"
            });

            // 1) Поиск одного слова
            var result1 = await documentService.SearchAsync("тестовый");
            Assert.Single(result1);
            Assert.Contains("**тестовый**", result1[0].HighlightedContent);

            // 2) Поиск фразы
            var result2 = await documentService.SearchAsync("несколько слов");
            Assert.Single(result2);
            Assert.Contains("**несколько** **слов**", result2[0].HighlightedContent);

            // 3) Проверка, что пустой результат возвращает 0
            var result3 = await documentService.SearchAsync("несуществующее");
            Assert.Empty(result3);
        }
        [Fact]
        public async Task SearchAsync_ShouldFindDifferentFormsOfWord()
        {
            var (adminService, documentService) = CreateServices();
            ...
            // добавляем документ с базовой формой слова
            await documentService.AddAsync(new DocumentModel
            {
                AdminId = admin.Id,
                Title = "Doc1",
                Author = "Author1",
                Year = "2025",
                Content = "Документ содержит слово 'тест'.",
                CloudLink = "link1"
            });

            // поиск с изменённой формой слова
            var result = await documentService.SearchAsync("тестом"); // окончание изменено

            // должен найти документ и подсветить исходное слово
            Assert.Single(result);
            Assert.Contains("**тест**", result[0].HighlightedContent);
        }
    }
}
